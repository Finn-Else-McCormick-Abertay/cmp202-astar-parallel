...
Path hashDistributedAStarSharedMemory(const DirectedGraph<ValueType, WeightType>& graph, int start, int goal, const Heuristic<ValueType, WeightType>& heuristicFunc) {
	...
	int numThreads; if (g_numThreads > 0) { numThreads = g_numThreads; }
	else { numThreads = std::thread::hardware_concurrency(); }
	...
	// Open sets are represented by a priority queue (ordered by lowest g + h score) protected by a mutex
